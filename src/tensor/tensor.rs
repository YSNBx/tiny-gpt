...
impl Tensor {
	pub fn new(data: Vec<f32>, shape: Vec<usize>) -> Self {
		...
		assert_eq!(data.len(), expected, "Data length {} != shape {:?} (expected {})", data.len(), shape, expected);
		Tensor { data, shape }
	}
	...
	pub fn reshape(&self, new_shape: Vec<usize>) -> Tensor {
		let shape1 = self.shape.iter().product::<usize>();
		let shape2 = new_shape.iter().product::<usize>();
		assert_eq!(shape1, shape2, "Sum of new shape {:?} != old shape {:?}", new_shape, self.shape,);
	...
	pub fn relu_forward(&self) -> Tensor {
		let data = self.data
		               .iter()
		               .map(|x| (*x).max(0.0))
		               .collect();
	...
	pub fn relu_backward(&self, pre_relu: &Tensor) -> Tensor {
		let data = self.data
		               .iter()
		               .zip(pre_relu.data.iter())
		               .map(|(grad, pre)| if *pre > 0.0 { *grad } else { 0.0 })
		               .collect();
		Tensor { data, shape: self.shape.clone() }
	}
	...
	fn assert_equal(&self, other: &Tensor) {
		assert_eq!(&self.shape, &other.shape, "Tensor shapes are not equal! {:?} != {:?}", &self.shape, other.shape);
	}

	fn assert_equal_dimensions(&self, other: &Tensor) {
		assert_eq!(self.shape[1], other.shape[0], "Inner dimensions must match: {} vs {}", self.shape[1], other.shape[0]);
	}
}
